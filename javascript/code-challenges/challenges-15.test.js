...
const screenForNames = (arr) => {
  // Solution code here...
  let Names = /^(Mr.|Mrs.|Ms.|Dr.)(\s)(\w{1,})/g;

  let Namecheck = arr.filter((str) => {
    return Names.test(str);
  });
  let check = Namecheck;
  return check;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let Highest = arr.filter((Max) => Max.mass > 77);
  return Highest.map((Name) => Name.name).join(" - ");
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  if (property === "price") {
    var ascendingPrice = arr.sort(
      (arr1, arr2) => Number(arr1.price) - Number(arr2.price)
    );
    return ascendingPrice;
  }
  if (property === "name") {
    var ascendingName = arr.sort((arr1, arr2) =>
      arr1.name.localeCompare(arr2.name)
    );
...
const detectTicTacToeWin = (board) => {
  ...
  let col2 = [board[0][1], board[1][1], board[1][1]];
  let col3 = [board[0][2], board[1][2], board[2][2]];
  let dia1 = [board[0][0], board[1][1], board[2][2]];
  let dia2 = [board[0][2], board[1][1], board[2][0]];
  let space = [row1, row2, row3, col1, col2, col3, dia1, dia2];
  for (let i = 0; i < space.length; i++) {
    if (space[i].every((item) => item == "O")) {
      return true;
    } else if (space[i].every((item) => item == "X")) {
      return true;
    }
  }
